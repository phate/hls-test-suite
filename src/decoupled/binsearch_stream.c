...
void kernel(
    ...
    for (uint32_t i = 0; i < table_elements; i++) {
        hls_stream_enq_uint32t(table_channel, table[i]);
    }
    for (uint32_t i = 0; i < table_elements; i += CHUNK_SIZE) {
        ...
        for (size_t j = 1; sorted_elements >= (j>>EXTRA_ITERATIONS); j = j << 1) {
            ...
            for (size_t k = 0; k < CHUNK_SIZE; ++k) {
                ...
                TYPE element;
                if (first_iteration) {
                    element = hls_stream_deq_uint32t(table_channel, LATENCY);
                    l = 0;
                    r = sorted_elements - 1;
                } else {
                    ...
                    TYPE tmp = hls_stream_deq_uint32t(sorted_channel, LATENCY);
                    m = (r + l) >> 1;
                    if (tmp > element) {
                        r = m;
                    } else {
                ...
                if (last_iteration) {
                    ...
                    hls_stream_enq_uint32t(r_sorted_channel, sorted[r]);
                    hls_stream_enq_uint32t(l_sorted_channel, sorted[l]);
//                    assert(r - l < 2);
                } else {
                    hls_stream_enq_uint32t(element_stream, element);
                    hls_stream_enq_uint32t(l_stream, l);
                    hls_stream_enq_uint32t(r_stream, r);
                    hls_stream_enq_uint32t(sorted_channel, sorted[m]);
                }
                iterations++;
            }
        }
    ...
    for (uint32_t i = 0; i < table_elements; i++) {
        int32_t res = -1;
        TYPE tmp_l = hls_stream_deq_uint32t(l_sorted_channel, LATENCY);
        TYPE tmp_r = hls_stream_deq_uint32t(r_sorted_channel, LATENCY);
