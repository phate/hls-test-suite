...
extern void hls_stream_enq_t32v4(uint32_t channel, bucket_vector data);
extern bucket_vector hls_stream_deq_t32v4(uint32_t channel, uint32_t buffer_slots);
...
#define OUTSTANDING_READS 128
    ...
    for (uint32_t i = 0; i < tableElements; ++i) {
        hls_stream_enq_uint32t(key_channel, table[i]);
    }
    ...
    for (uint32_t j = 0; j < tableElements;){
        ...
        if (rif < OUTSTANDING_READS && key_i < tableElements){
            i = key_i;
            key = hls_stream_deq_uint32t(key_channel, OUTSTANDING_READS);
            key_i++;
            idx = key & hashMask;
        } else if(rif) {
            i = hls_stream_deq_uint32t(i_stream2, OUTSTANDING_READS);
            key = hls_stream_deq_uint32t(key_stream2, OUTSTANDING_READS);
            bucket_vector b = hls_stream_deq_t32v4(bucket_channel, OUTSTANDING_READS);
            rif--;
            if (key == b[0]) {
                result[i] = b[1];
                j++;
        ...
        hls_stream_enq_t32v4(bucket_channel, hashtable[idx]);
