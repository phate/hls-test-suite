...
void kernel(
    ...
    for (uint32_t j = startElement; j < endElement; j++) {
        uint32_t i = hls_stream_deq_uint32t(cols_dec_channel, 2);
    ...
    for (uint32_t j = startElement; j <= endElement; j++) {
        while (j==nextDelim){
            ...
            sum = 0;
            nextDelim = hls_stream_deq_uint32t(rows_dec_channel, 2);
        }
        if(j < endElement){
            TYPE cval = hls_stream_deq_TYPE(vals_dec_channel, 2);
            TYPE vval = hls_stream_deq_TYPE(vec_dec_channel, 2);
            TYPE Si = cval * vval;
            sum += Si;
        }
    }
